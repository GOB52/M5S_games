...
struct obj_info_t
{
    ...
    static uint32_t _width, _height; // lcd width/height
};
uint32_t obj_info_t::_width = 0;
uint32_t obj_info_t::_height = 0;
...
namespace
{
constexpr unsigned short infoWidth = 32;
constexpr unsigned short infoHeight = 32;
constexpr unsigned short alertWidth = 32;
constexpr unsigned short alertHeight = 32;
constexpr unsigned short closeWidth = 32;
constexpr unsigned short closeHeight = 32;
...
void m5s_app::setup(LGFX* lcd)
{
    ...
    obj_info_t::_width = _lcd_width = _lcd->width();
    obj_info_t::_height = _lcd_height = _lcd->height();
...
void m5s_app::render()
{
    ...
    {
        _flip = !_flip;
        _sprites[_flip].clear();
        for (size_t i = 0; i != obj_count; i++)
        {
            a = &objects[i];
            icons[a->img].pushRotateZoom(&_sprites[_flip], a->x, a->y - y, a->r, a->z, a->z, 0);
        }
        if (y == 0)
        {
            _sprites[_flip].setCursor(0, 0);
            _sprites[_flip].setFont(&fonts::Font4);
            _sprites[_flip].setTextColor(0xFFFFFFU);
            _sprites[_flip].printf("obj:%d  fps:%2.2f", obj_count, fps());
        }
